...
d_cancer_acc[, month := as.numeric(month(acc_startdate))]
d_cancer_acc[, month := as.factor(month)] # control for, option 1
table(d_cancer_acc$month)

d_cancer_acc[, day := as.numeric(mday(acc_startdate))]
d_cancer_acc[, day_of_year := yday(acc_startdate)] # option 2

d_cancer_acc[, season := fifelse(
...
d_cancer_acc[, season := factor(season, levels = c("Spring", "Summer", "Autumn", "Winter"))]

d_cancer_acc[, cancer_time_since_diag_other_f := cut(as.numeric(icd_ii_time_since_lo),
...
d_cancer_acc[, cancer_time_since_diag_other_f := as.character(cancer_time_since_diag_other_f)]
d_cancer_acc[, cancer_time_since_diag_other_f := ifelse(cancer_other_before_acc == "Healthy" | cancer_time_since_diag_other_f == "Healthy", "Healthy", cancer_time_since_diag_other_f)]
d_cancer_acc[, cancer_time_since_diag_other_f := ifelse(cancer_other_before_acc == "Others", "Others", cancer_time_since_diag_other_f)]

d_cancer_acc[, cancer_time_since_diag_other_f := as.factor(cancer_time_since_diag_other_f)]
d_cancer_acc[, cancer_time_since_diag_other_f := relevel(cancer_time_since_diag_other_f, ref = "Healthy")]

d_cancer_acc[, cancer_time_since_diag_f := ifelse(cancer_time_since_diag_other_f %in% c("Others"), NA, as.character(cancer_time_since_diag_other_f))]

d_cancer_acc[, cancer_time_since_diag_f := as.factor(cancer_time_since_diag_f)]
d_cancer_acc[, cancer_time_since_diag_f := relevel(cancer_time_since_diag_f, ref = "Healthy")]

table(d_cancer_acc$cancer_time_since_diag_other_f)
table(d_cancer_acc$cancer_time_since_diag_f)
str(d_cancer_acc$cancer_time_since_diag_f)
str(d_cancer_acc$season)
...
clr_cancer_acc_sub <- complr(
  data = d_cancer_acc[cancer_before_acc_type_other %in% c("Healthy", "Blood", "Gynaecological", "Breast", "Colorectal", "Prostate", "Other Skin")],
  transform = "ilr",
  parts = c("sleep_comp", "mvpa_comp", "lpa_comp", "sb_comp"),
  sbp = sbp,
  total = 1440
...
clr_cancer_acc_one_primary <- complr(
  data = d_cancer_acc[cancer_before_acc_type_other %nin% c("Multiple Primary", "Others")],
  transform = "ilr",
  parts = c("sleep_comp", "mvpa_comp", "lpa_comp", "sb_comp"),
  sbp = sbp,
  total = 1440
